...
echo -n "${video_file_counter} at " >> /var/spool/vector/run_out
...
bash  -x ${MY_PATH}/file_usb_push.sh ${video_file_counter} >> /var/spool/vector/run_out_${counterPart} 2>&1

echo -n "${video_file_counter} completed at " >> /var/spool/vector/run_out
date >> /var/spool/vector/run_out
echo; echo; echo
